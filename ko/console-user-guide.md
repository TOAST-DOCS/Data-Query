...
!!! tip "알아두기"
    * Data Lake Storage는 DataQuery와 다른 NHN Cloud 프로젝트에 존재할 수 있습니다.
...
* **데이터 소스 추가**를 클릭한 뒤 데이터 소스 추가 페이지에서 Object Storage 정보를 입력합니다.
    ...
    * 액세스 키, 비밀 키, 리전
        ...
        * 리전은 NHN Cloud 리전에 대응하는 S3 리전 이름을 선택해야 합니다. Object Storage 가이드의 [AWS 명령줄 인터페이스(CLI) > 설정](https://docs.toast.com/ko/Storage/Object%20Storage/ko/s3-api-guide/#aws-cli)을 참고하세요.
    * 버킷 이름
    ...
    * 메타스토어 타입
        * Object Storage에 데이터를 입/출력하기 위해 사용하는 메타스토어의 인스턴스 타입을 설정할 수 있습니다.
            * 타입에 대한 설명은 [서비스별 요금](https://www.nhncloud.com/kr/pricing/by-service?c=Data%20%26%20Analytics&s=DataQuery)에서 확인할 수 있습니다.
        * 기본으로 c1m3 타입이 선택되어 있습니다.
        * 설정은 다음 클러스터 시작 이후에 적용됩니다. 클러스터가 이미 켜진 상태라면 종료한 이후 다시 시작해야 합니다.
    * 추가 설정 정보
...
* 접속 URL
    ...
    * 타임존 처리가 필요할 경우 serverTimezone 파라미터를 설정해야 합니다.
        * 예: jdbc:mysql://localhost:10000?**serverTimezone=Asia/Seoul**
* 사용자 ID
    * 접속할 MySQL 계정명입니다.
* 비밀번호
    * 접속할 MySQL 비밀번호입니다.
...
* 비밀번호
    * 접속할 Oracle 비밀번호입니다.
* 추가 설정 정보
    * 미지원 타입 처리: 지원하지 않는 칼럼 데이터 타입의 처리 방법을 정할 수 있습니다.
...
* 접속 URL
    ...
    * 타임존 처리가 필요할 경우 serverTimezone 파라미터를 설정해야 합니다.
        * 예: jdbc:mariadb://localhost:10000?**serverTimezone=Asia/Seoul**
* 사용자 ID
    * 접속할 MariaDB 계정명입니다.
* 비밀번호
    * 접속할 MariaDB 비밀번호입니다.
...
* 자세한 사항은 Trino의 가이드 문서를 참고하세요.
    * [키워드, 데이터 타입](https://trino.io/docs/476/language.html)
...
* 쿼리 편집기에서 실행된 쿼리 결과를 확인할 수 있습니다.
    * 쿼리 결과를 최대 30MB까지 제공합니다.
        * 30MB를 초과한 이후의 결과는 조회 및 다운로드할 수 없습니다.
        * Trino를 직접 연동(예: JDBC, CLI)하면 쿼리 수행 결과 전체 데이터를 얻을 수 있습니다. 설정 메뉴 가이드를 참고하세요.
    * 결과 영역은 쿼리 결과를 5,000행 단위로 불러옵니다.
        * 결과가 5,000행보다 많은 경우 **더 불러오기**를 클릭하여 추가 로드할 수 있습니다.
    * **클립보드에 복사**를 클릭하여 결과 영역에 보이는 쿼리 결과를 클립보드에 저장할 수 있습니다.
    * 쿼리 결과는 쿼리 완료 시각부터 최대 7일까지 다운로드할 수 있습니다. 12월 1일 13시 53분 32초에 쿼리 실행이 완료될 경우 12월 8일 13시 53분 32초 전까지 다운로드할 수 있습니다.
    * 결과 영역에서 마우스 오른쪽 버튼을 클릭하여 콘솔 쿼리 결과 복사, 내보내기를 실행할 수 있습니다.
...
* 엔드포인트에 접속하려면 개인별 인증 정보가 필요하며, **설정** 메뉴에서 **인증 키** 발급을 클릭해 발급받을 수 있습니다.
    ...
    * 비밀번호(인증 키)는 최초 발급 시에만 확인할 수 있습니다. 인증 키를 분실할 경우 **인증 키 재발급**을 클릭해 새 인증 키를 발급받아야 합니다.
...
* DataQuery에서는 Object Storage 접근을 위해 S3 호환 레이어를 사용하며, 스키마 또는 테이블의 데이터 경로 지정 시 s3a 프로토콜을 사용해야 합니다(예: s3a://example/test).
* Object Storage상의 Parquet, JSON, ORC, CSV, Text 타입의 데이터를 처리할 수 있습니다.
...
-- Managed Table 샘플
 CREATE TABLE sample (...);
-- External Table 샘플
 CREATE TABLE sample (...) WITH ( external_location = 's3a://<버킷,컨테이너명>/<데이터 디렉터리 경로>/');
```
...
* 파티션 프로시저
  * sync_partition_metadata
    ...
    * Object Storage 오브젝트들의 경로를 기준으로 Hive 파티션 값을 유추하는 방법은 아래와 같습니다.
      ...
      * 파티션 칼럼이 c1 칼럼과 c2 칼럼으로 지정되어 있을 경우, Hive 파티션으로 유효한 경로는 `/c1=<c1 값>/c2=<c2 값>`를 포함해야 합니다.
    ...
    * 주의
      * sync_partition_metadata를 실행하려면 테이블에서 정의한 external\_location에 컨테이너 하위 경로가 반드시 하나 더 존재해야 합니다. 예를 들어, external\_location='s3a://location/tmp/'와 같이 컨테이너가 location일 때 하위에 tmp로 경로가 하나 더 있어야 합니다.
  * register_partition
    * 사용자가 지정한 경로를 파티션의 값으로 직접 등록할 수 있습니다.
    * 세 번째 파라미터인 partition_columns에는 Hive 테이블에서 정의한 파티션 칼럼을 입력합니다.
...
* 제약 사항
    ...
    * DataQuery에서는 Object Storage 접근을 위해 S3 호환 레이어를 사용하며, 스키마 또는 테이블의 데이터 경로 지정 시 s3a 프로토콜을 사용해야 합니다(예: s3a://example/test).
    * External table의 external\_location으로 지정되는 데이터 디렉터리 경로 객체가 별도로 존재해야 합니다.
        * Object Storage에서 디렉터리를 별도로 생성한 상태에서 데이터가 해당 디렉터리에 위치해야 정상적으로 연동이 될 수 있습니다.
        * 만약 해당 처리에 문제가 있을 경우에는 고객지원으로 문의하세요.
...
* 제약 사항
    * DELETE는 특정 조건이 충족될 때만 제한적으로 수행할 수 있습니다.
        ...
        * 텍스트 타입의 칼럼은 푸시다운이 지원되지 않습니다.
        * [상세 정보](https://trino.io/docs/476/connector/mysql.html#predicate-pushdown-support)
...
* 제약 사항
    * DELETE는 특정 조건이 충족될 때만 제한적으로 수행할 수 있습니다.
        ...
        * [상세 정보](https://trino.io/docs/476/connector/postgresql.html#predicate-pushdown-support)
    * UPDATE는 특정 조건이 충족될 때만 제한적으로 수행할 수 있습니다.
        * 상수 값으로의 할당 및 조건자(Predicate)가 존재할 경우에만 수행할 수 있습니다.
        * 산술 표현식, 함수 호출 및 상수가 아닌 값으로의 UPDATE문은 지원되지 않습니다.
        * 테이블의 모든 칼럼을 동시에 업데이트할 수 없습니다.
...
* 제약 사항
    * DELETE, UPDATE는 특정 조건이 충족될 때만 제한적으로 수행할 수 있습니다.
        * where 절이 존재할 때 조건자(Predicate)가 데이터 소스로 온전히 푸시다운(Pushdown)될 수 있어야 합니다.
        * CLOB, NCLOB, BLOB, 또는 RAW(n)인 Oracle 타입의 칼럼은 푸시다운이 지원되지 않습니다.
        * [상세 정보](https://trino.io/docs/476/connector/oracle.html#predicate-pushdown-support)
    * UPDATE는 특정 조건이 충족될 때만 제한적으로 수행할 수 있습니다.
        * 상수 값으로의 할당 및 조건자(Predicate)가 존재할 경우에만 수행할 수 있습니다.
        * 산술 표현식, 함수 호출 및 상수가 아닌 값으로의 UPDATE문은 지원되지 않습니다.
        * 테이블의 모든 칼럼을 동시에 업데이트할 수 없습니다.
...
* 제약 사항
    * DELETE는 특정 조건이 충족될 때만 제한적으로 수행할 수 있습니다.
        * where 절이 존재할 때 조건자(Predicate)가 데이터 소스로 온전히 푸시다운(Pushdown)될 수 있어야 합니다.
        * 텍스트 타입의 칼럼은 푸시다운이 지원되지 않습니다.
        * [상세 정보](https://trino.io/docs/476/connector/mariadb.html#predicate-pushdown-support)
    * UPDATE는 특정 조건이 충족될 때만 제한적으로 수행할 수 있습니다.
        * 상수 값으로의 할당 및 조건자(Predicate)가 존재할 경우에만 수행할 수 있습니다.
        * 산술 표현식, 함수 호출 및 상수가 아닌 값으로의 UPDATE문은 지원되지 않습니다.
        * 테이블의 모든 칼럼을 동시에 업데이트할 수 없습니다.
...
* 테이블 속성
    * 테이블의 메타데이터를 설정할 수 있습니다. [추가 정보](https://trino.io/docs/476/connector/iceberg.html#table-properties)

| 속성 이름 | 설명 |
| ----- | --- |
| format | 테이블 데이터 파일의 형식을 지정합니다. PARQUET, ORC, 또는 AVRO. <br> 기본값은 PARQUET입니다. |
| partitioning | 테이블 파티션을 지정합니다. 테이블이 c1 칼럼과 c2 칼럼으로 분할된 경우 partitioning=ARRAY['c1', 'c2']로 지정할 수 있습니다. |
...
* 테이블 속성을 통해 테이블 데이터가 분할 저장된 구조의 분할된(파티션된) 테이블을 만들 수 있습니다.
    * 파티션 칼럼이 c1 칼럼과 c2 칼럼으로 지정되어 있을 경우, 해당 파티션의 데이터는 테이블 데이터 경로 하위의 `/c1=<c1 값>/c2=<c2 값>`에 저장됩니다.
* Iceberg는 쓰기(write)된 데이터의 값을 통해 파티션을 자동으로 관리해 주기 때문에 파티션을 수동으로 추가/관리할 수 없습니다.
* 테이블 칼럼을 이용(변환)하여 파티션을 지정할 수 있는 기능을 지원합니다.
    * year, month, day, hour, bucket, truncate [추가 정보](https://trino.io/docs/476/connector/iceberg.html#partitioned-tables)
...
* 스키마 진화(schema evolution)
    * Iceberg는 메타데이터만을 변경하는 스키마 진화를 지원합니다. 스키마 업데이트를 수행할 때 데이터 파일 자체가 변경되지 않습니다.
    * 칼럼 추가, 삭제, 재정렬, 이름 변경, 타입 변경을 지원합니다.
...
* add_files 프로시저
  ...
  | recursive_directory | FAIL(기본 값), TRUE, FALSE   | location 이하 경로를 재귀로 탐색할 수 있을 때의 동작<br>FAIL => 입력한 데이터 파일 경로가 2단계 깊이로 재귀 탐색이 가능하다면 쿼리가 실패합니다.<br> TRUE => 입력한 데이터 파일 경로 하위를 재귀로 모두 탐색합니다.<br>FALSE => 입력한 데이터 파일 경로 2단계 깊이부터는 무시합니다. |
...
* add_files_with_partition 프로시저
  ...
  * 등록하려는 파티션 칼럼 타입이 DATE일 때는 `YYYY-MM-DD`, TIMESTAMP일 때는`YYYY-MM-DD HH:mm:ss`의 형식으로 입력해야 합니다. timezone이 있는 TIMESTAMP인 경우 `YYYY-MM-DD HH:mm:ss Asia/Seoul`과 같이 끝에 zoneId가 명시되어야 합니다.

    |인자  | 지원하는 값                    | 설명                                                                                                                                                                                      |
    | --- |---------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    | location | 데이터 파일 경로                 | 추가하려는 데이터 파일 경로                                                                                                                                                                         |
    |partition_columns| ARRAY['partition_column'] | iceberg 테이블에 정의된 파티션 칼럼을 나열하며 테이블이 c1, c2 칼럼으로 분할된 경우 ARRAY['c1', 'c2']로 입력                                                                                                                 |
    |partition_values| ARRAY['partition_value']  | 등록하려는 파티션 값                                                                                                                                                                             |
    | format | PARQUET(기본 포맷), ORC, AVRO | 추가하려는 데이터 파일의 포맷                                                                                                                                                                        |
    | recursive_directory | FAIL(기본 값), TRUE, FALSE   | location 이하 경로를 재귀로 탐색할 수 있을 때의 동작<br>FAIL => 입력한 데이터 파일 경로가 2단계 깊이로 재귀 탐색이 가능하다면 쿼리가 실패합니다.<br> TRUE => 입력한 데이터 파일 경로 하위를 재귀로 모두 탐색합니다.<br>FALSE => 입력한 데이터 파일의 경로 2단계 깊이부터는 무시합니다. |
    |duplicate_file  | FAIL(기본 값), SKIP, ADD     | 등록하려는 데이터 파일이 이미 등록된 iceberg 테이블의 데이터 파일과 중복일 때의 동작<br>FAIL => iceberg 테이블에 이미 등록된 데이터 파일과 비교해서 중복된 데이터 파일이 있으면 쿼리 실패합니다.<br>SKIP => 중복된 파일은 무시합니다.<br>ADD => 데이터 파일을 추가합니다.            |
```sql
-- 파티션 칼럼이 year이면서 day 변환이 적용된 iceberg 테이블
ALTER TABLE example.system.example_table
...
# 예: chmod +x trino-cli-476-executable.jar

./trino-cli-476-executable.jar --server <접속URL(필수)> \
...
* 설정 파라미터
    * 접속 URL(필수)
        * 설정 화면에서 제공받은 접속 URL(예: [https://x-x-x-x-x.kr1-cluster-dataquery.nhncloudservice.com](https://x-x-x-x-x.kr1-cluster-dataquery.nhncloudservice.com))
    * 아이디(필수)
        * 인증 정보 화면에서 제공받은 아이디
    * 비밀번호(필수)
        * 인증 정보 화면에서 제공받은 비밀번호
...
* catalog, schema 값은 명령을 수행할 연결의 값으로, 입력하지 않아도 CLI를 실행할 수 있으며 아래 쿼리로 catalog나 schema 목록을 확인할 수 있습니다.
    * show catalogs
    * show schemas
...
* 설정 파라미터
    * host(필수)
        * 설정 화면에서 제공받은 접속 URL에서 `https://`를 제외한 나머지를 입력합니다(예: x-x-x-x-x.kr1-cluster-dataquery.nhncloudservice.com).
    * port(필수)
        * 443을 입력합니다.
    * catalog
        * 연결을 원하는 데이터 소스 이름
...
!!! tip "알아두기"
    * **예약된 쿼리**의 결과는 UI에 출력되거나, API 응답에 포함되지 않습니다. 쿼리의 결과가 필요한 경우 CTAS 또는 INSERT SELECT 쿼리를 사용하여 별도의 테이블에 저장해야 합니다.
    * User Access Key 및 Secret Access Key는 **API 보안 설정** > **User Access Key 생성**을 사용하여 발급할 수 있습니다.
    * 대상 템플릿을 사용한 일정 생성은 사용자 가이드의 **Application Service > Cloud Scheduler > 콘솔 사용 가이드**를 참고하세요.

!!! danger "주의"
    * 쿼리를 실행하기 위해서는 클러스터가 켜져 있어야 합니다.
    * 쿼리를 실행하기 위해서는 DataQuery 서비스의 **인증 정보**를 발급한 상태여야 합니다.
