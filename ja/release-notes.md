...
* データソースの種類に Data Lake Storage を追加しました。
...
* Cloud Scheduler サービスに「予約されたクエリ」テンプレートを追加しました。
...
* コンソールのクエリ結果表示ポリシーの改善
  * コンソールで実行したクエリ結果を 1MB、5,000 行以内で表示していた制限を削除しました。
  * コンソールで実行したクエリ結果を最大 30MB まで表示できるように改善しました。
  * コンソールのクエリ結果を照会およびコピーするボタンを追加しました。

<a id="september-23-2025"></a>
## 2025. 09. 23. { #september-23-2025 }

<a id="trino-version-upgrade"></a>
### Trino バージョンアップグレード { #trino-version-upgrade }

* DataQuery を Trino 476 バージョンを基盤にサービスするようにアップグレードしました。
* 一部のクエリのパフォーマンス向上とバグ修正が含まれます。
...
* クラスタの状態指標可視化領域を追加しました。
  * 2025 年 6 月 24 日以降に起動したクラスタの状態を収集します。
...
* Object Storage データソースと連携するためのメタストアのインスタンスタイプを設定できる機能を追加しました。
...
* DataQuery を Trino 462 バージョンを基盤にサービスするようにアップグレードしました。
* Iceberg コネクタの add_files_with_partition 関数が追加されました。

<a id="october-29-2024"></a>
## 2024. 10. 29. { #october-29-2024 }

<a id="october-29-2024-trino-version-upgrade"></a>
### Trino バージョンアップグレード { #october-29-2024-trino-version-upgrade }

* DataQuery を Trino 455 バージョンを基盤にサービスするようにアップグレードしました。
* 一部のクエリのパフォーマンス向上とバグ修正が含まれます。

<a id="october-29-2024-added-features"></a>
### 機能追加 { #october-29-2024-added-features }

* データソースの種類に Iceberg が追加されました。
...
* クエリ履歴保存のための Object Storage 認証情報の有効期限切れ時に連携無効化の案内メールが送信されるように改善しました。
...
* Object Storage データソースの最大登録制限を 5 個に変更しました。
* Object Storage データソースの最小登録制限を削除しました。
...
* ユーザーがよく使用するクエリを保存および管理できる機能が追加されました。

<a id="january-23-2024"></a>
## 2024. 01. 23. { #january-23-2024 }

<a id="january-23-2024-trino-version-upgrade"></a>
### Trino バージョンアップグレード { #january-23-2024-trino-version-upgrade }

* DataQuery が提供する Trino バージョンが 398 から 434 にアップグレードされました。
* データソースの種類に PostgreSQL、Oracle、EDB が追加されました。
...
* 分散SQLクエリエンジン Trino を基盤に、大規模なデータに対してクエリを実行できるサービスです。
* データソースとして NHN Cloud Object Storage、NHN Cloud RDS for MySQL をサポートします。