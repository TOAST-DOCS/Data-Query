DataQueryサービスを使用するにはデータソースの追加が必須です。
基本的に以下のような手順でサービスを使用できます。
...
* データソース設定および反映制約事項
    ...
    * Object Storageタイプのデータソースが存在する時のみ**クラスタの有効化およびクエリ実行が可能です。**
* **データソース追加**ボタンをクリックします。

#### Object Storageデータソースタイプ

* **データソース追加**ボタンを押して表示されるポップアップでObject Storage関連情報を入力します。
    * データソース名
        * クエリ実行時に使用されるセパレータで、データソース間で一意の値なければなりません。
    * アクセスキー、秘密鍵、エンドポイント
        * 連動するデータが存在するObject Storageの接続情報です。
        * アクセスキーと秘密鍵はObject Storageコンソール内で発行できます。 ([ガイド参考](https://docs.toast.com/ko/Storage/Object%20Storage/ko/console-guide/#s3-api))
        * エンドポイントはObject Storageガイドのリージョン別S3 APIエンドポイントをご覧ください。([ガイド参考](https://docs.toast.com/ko/Storage/Object%20Storage/ko/s3-api-guide/#aws-cli))
    * バケット名
        * システムで基本テーブル情報や管理テーブル情報、データを保存するために使用するObject Storageコンテナ名(dataquery-warehouse)です。
            * 独自にdataquery-warehouseコンテナを作成して使用します。
        * 連動する既存データがdataquery-warehouseコンテナ内部に存在しなくても構いません。
* その他の事項
    * 連動するObject Storageは、同じNHN Cloudプロジェクト内に存在しなくても構いません。
...
* クエリエディタはクラスタ管理、スキーマ、エディタ、結果領域に区分されます。
...
* クラスタをOn/Offできます。
* 追加、変更、削除したデータソース情報を実際の動作に反映させるにはDataQueryクラスタを**再起動**する必要があります。
...
* 継続的にクラスタのオン/オフに失敗する場合はお問い合わせください。

### スキーマ領域

* 接続設定されたデータソースと、該当ソースで提供する実際のDB 、テーブル、カラム情報を確認できます。
    * information\_schemaはデータソースとの接続情報を持っているDBで、ユーザーが任意にデータを操作できません。
* 各**更新ボタン**を押してデータソース、スキーマ、テーブル、カラム情報を新しく取得できます。
    * ただし、上位スキーマを更新しても下位情報まで全部新しく読み込みません。
    * ex)テーブルを更新する場合、テーブルリストのみ新しく取得し、各テーブルのカラム情報はアップデートされません。

### エディタ領域

* **クエリ追加ボタン**を押してクエリエディタを最大10個まで作成できます。
* 実行ボタン、またはctrl + enterの組み合わせでクエリを実行でき、エディタの下部で実行中のクエリの進行度、失敗時の原因ログを確認できます。
* クエリ作成時に収集されたデータソース、スキーマ、テーブル、カラム名のオートコンプリートをサポートしています。

#### SQLガイド

* DataQueryのSQLはTrino基準に従って動作します。
    * Trinoは標準SQL文法(ANSI SQL)に従います。
    * Trino独自にデータを持たず、複数のデータソースとの連動によりデータを処理します。
        * データソースごとに例外的な文法や制約事項が存在する場合があります。
    * トランザクション処理(OLTP)作業ではなく、分析処理(OLAP)作業に合うように設計されています。
* データ型
...
* Trinoクエリ
    * DDL(Data Definition Language、データ定義語)、DML(Data Manipulation Language、データ操作語)をサポートし、以下の追加構文をサポートします。
        * メタ情報を確認できる構文：SHOW CATALOGS、SHOW SCHEMAS、SHOW TABLES、SHOW STATS FOR
        * システムの内蔵プロシージャ(Procedure)を確認したりクエリの実行計画を確認できる構文：CALL、EXPLAIN
...
* クエリエディタで実行されたクエリ結果を確認できます。
    * データサイズに応じて**最大5,000個の**制限された結果を提供しています。
    * クエリ結果のダウンロードは最大30MBまで提供します。
        * Trinoを直接連動(ex. JDBC、CLI)する場合は、クエリ実行結果全体データを取得できます。設定メニューガイドをご覧ください。
        * クエリ結果のダウンロードはクエリ完了時刻から最大7日までダウンロードが可能です。
            * ex) 12月1日13時53分32秒にクエリ実行が完了した場合、12月8日13時53分32秒の前までダウンロード可能
    * マウス右クリックで表示されるメニューからコンソールクエリ結果のコピー、エクスポートが可能です。
* クエリエディタで実行したクエリリストを提供します。
* クエリ実行リストをクリックすると、そのクエリが入力されたクエリウィンドウが追加作成されます。
...
* 一番右にある展開ボタンを押すと実行したクエリの追加の実行情報が表示され、ダウンロードボタンを押してクエリの全体実行情報をダウンロードできます。
...
* エンドポイント接続には、個人別の認証情報が必要で、設定メニューで認証キー発行ボタンを押して発行できます。
    * 個人別の認証情報は、IDと認証キーで構成されており、エンドポイントに接続する時、ユーザー名(ID)とパスワード(認証キー)として活用されます。
    * パスワード(認証キー)は、最初の発行時にのみ確認できます。認証キーを忘れた場合は認証キー再発行ボタンを押して新たに認証キーを発行する必要があります。
    * 発行/再発行された認証キーは、発行してから5分後に使用可能です。
...
* Object Storageデータソースに対するクエリは、Trino-Hiveに基づいて実行されます。
    * Hiveは[Apache Hadoop](https://hive.apache.org/)分散ストレージ環境上でSQL作業処理をサポートするためのソリューションです。
* DataQueryではObject StorageアクセスのためにS3互換レイヤーを使用しており、スキーマまたはテーブルのためのデータパス指定時、**s3a**プロトコルを使用する必要があります(例s3a://example/test)
...
* テーブルの種類
    * 内部テーブル(Managed Table)
        ...
        * テーブルを削除するとテーブル情報およびObject Storage上に保存されたファイルが一緒に削除されます。
    ...
    * 外部テーブル(External Table)
        ...
        * テーブルを作成する時、WITH節のexternal\_locationにデータのパスを指定する必要があります。
          * external_locationデータのパス入力時、バケット、コンテナ名は[NHN Cloud Object Storageのバケット命名ルール](https://docs.toast.com/ko/Storage/Object%20Storage/ko/s3-api-guide/#_7)を守る必要があります。
...
* 設定メニューから発行された認証情報、接続情報、TrinoでサポートするCLIツールを利用してコマンドラインからクエリ実行をできます。
    * [Trino CLI](https://repo1.maven.org/maven2/io/trino/trino-cli/398/trino-cli-398-executable.jar)

```
# ファイルに実行権限が必要です。 chmod +x 로 付与できます。
...
* catalog、schema値はコマンドを実行する接続に対する値で、入力しなくてもcli実行は可能で、以下のクエリでcatalogやschemaリストを確認できます。
    * show catalogs
    * show schemas
* 詳細はTrino Webサイトをご覧ください。
    * [ガイドページ](https://trino.io/docs/current/client/cli.html)

### JDBC接続

* 設定メニューから発行された認証情報、接続情報、TrinoでサポートするJDBCドライバーを利用してJDBC接続を行うことができます。
...
* 設定パラメータ
    * host(**必須**)
        * 設定画面で提供された接続URLで、https:// 以外の部分を入力します。(ex . [x-x-x-x-x.cluster-dataquery.cloud.toast.com](http://x-x-x-x-x.cluster-dataquery.cloud.toast.com))
    * port(**必須**)
        * 443を入力します。
    * catalog
        * 接続したいデータソース名
...
* 詳細はTrino Webサイトをご覧ください。
    * [Trino JDBCガイドページ](https://trino.io/docs/398/client/jdbc.html)
