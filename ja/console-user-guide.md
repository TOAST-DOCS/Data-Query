...
## Data & Analytics > DataQuery > コンソール使用ガイド { #data-analytics-dataquery-console-user-guide }
...
### データソース追加 { #add-data-source }

* データソース設定および反映制約事項
    * Data Lake StorageとObject Storageタイプのデータソースは、合計で最大5つまで登録できます。
...
### Data Lake Storageデータソースのタイプ { #data-lake-storage-data-source-type }

データソース追加ページの**データソースのタイプ**で**Data Lake Storage**を選択し、接続情報と追加設定情報を入力した後、**追加**してData Lake Storageデータソースを作成できます。
...
### MySQLデータソースタイプ { #mysql-data-source-type }
...
* 接続URL
    ...
    * タイムゾーン処理が必要な場合、serverTimezoneパラメータを設定する必要があります。
        * 例：jdbc:mysql://localhost:10000?**serverTimezone=Asia/Seoul**
* ユーザーID
    * 接続するMySQLアカウント名です。
* パスワード
    * 接続するMySQLパスワードです。
...
### PostgreSQLデータソースタイプ { #postgresql-data-source-type }
...
### Oracleデータソースタイプ { #oracle-data-source-type }
...
* パスワード
    * 接続するOracleパスワードです。  
* 追加設定情報
    * 未サポートタイプ処理：サポートしないカラムのデータタイプの処理方法を設定できます。
...
### EDBデータソースタイプ { #edb-data-source-type }
...
### MariaDBデータソースタイプ { #mariadb-data-source-type }
...
* 接続URL
    ...
    * タイムゾーン処理が必要な場合serverTimezoneパラメータを設定する必要があります。
        * 例：jdbc:mariadb://localhost:10000?**serverTimezone=Asia/Seoul**
* ユーザーID
    * 接続するMariaDBアカウント名です。
* パスワード
    * 接続するMariaDBパスワードです。
...
### Icebergデータソースタイプ { #iceberg-data-source-type }
...
* 詳細については、Trinoのガイド文書をご覧ください。
    * [キーワード、データ型](https://trino.io/docs/476/language.html)
...
### 5. 結果/コンソール実行クエリ領域 { #resultsconsole-execution-query-area }

* クエリエディタで実行されたクエリ結果を確認できます。
    * クエリ結果を最大30MBまで提供します。
        * 30MBを超過した以降の結果は、照会及びダウンロードできません。
        * Trinoを直接連携(例：JDBC、CLI)すると、クエリ実行結果の全データを取得できます。設定メニューガイドを参照してください。
    * 結果領域は、クエリ結果を5,000行単位で読み込みます。
        * 結果が5,000行より多い場合、**さらに読み込む**をクリックして追加ロードできます。
    * **クリップボードにコピー**をクリックし、結果領域に表示されるクエリ結果をクリップボードに保存できます。
    * クエリ結果は、クエリ完了時刻から最大7日までダウンロードできます。12月1日13時53分32秒にクエリ実行が完了した場合、12月8日13時53分32秒までダウンロードできます。
    * 結果領域で右クリックし、コンソールクエリ結果のコピー、エクスポートを実行できます。
...
### 外部連動 { #external-integration }

* 外部ツール(JDBC、CLI、BIソリューションなど)と連携するようにTrinoエンドポイントを提供し、**設定**ページで提供する情報を利用して連携できます。
...
### Object Storageデータソースクエリ実行 { #run-object-storage-data-source-query }

* Object Storageデータソースクエリは、Trino-Hiveに基づいて実行されます。
    * Hiveは[Apache Hadoop](https://hive.apache.org/)分散ストレージ環境でSQL作業処理をサポートするためのソリューションです。
* DataQueryではObject StorageアクセスのためにS3互換レイヤーを使用し、スキーマまたはテーブルのためのデータパス指定時、s3aプロトコルを使用する必要があります(例：s3a://example/test)。
...
* Trino-Hiveは基本的に標準SQL文法に従いますが、Hive動作対応のための追加機能/文法が存在します。 [詳細情報](https://trino.io/docs/476/connector/hive.html)
* サポートデータフォーマット
...
* パーティションプロシージャ
  * sync_partition_metadata
    ...
    * Object Storageオブジェクトのパスを基準にHiveパーティションの値を推論する方法は次のとおりです。
      ...
      * パーティションカラムがc1カラムとc2カラムで指定されている場合、Hiveパーティションとして有効なパスは`/c1=<c1 value>/c2=<c2 value>`を含む必要があります。
  ...
  * register_partition
    ...
    * 3番目のパラメータであるpartition_columnsには、Hiveテーブルで定義したパーティションカラムを入力します。
...
### MySQLデータソースクエリ実行 { #execute-mysql-data-source-query }
...
* 制約事項
    * DELETEは特定の条件が満たされた場合のみ制限的に実行できます。
        ...
        * テキストタイプのカラムはプッシュダウンがサポートされません。
        * [詳細情報](https://trino.io/docs/476/connector/mysql.html#predicate-pushdown-support)
...
### PostgreSQLデータソースクエリの実行 { #execute-postgresql-data-source-query }

* PostgreSQLデータソースクエリは、Trino-PostgreSQLに基づいて実行されます。
...
* 制約事項
    * DELETEは特定の条件が満たされた場合のみ制限的に実行できます。
        ...
        * [詳細情報](https://trino.io/docs/476/connector/postgresql.html#predicate-pushdown-support-limitation)
    * UPDATEは、特定の条件が満たされる場合のみ制限的に実行できます。
        * 定数値への割り当てと術語(Predicate)が存在する場合のみ実行することができます。
        * 算術式、関数呼び出しおよび定数以外の値へのUPDATE文はサポートされません。
        * テーブルの全てのカラムを同時に更新することはできません。
...
### Oracleデータソースクエリの実行 { #execute-oracle-data-source-query }
...
* 制約事項
    * DELETE, UPDATEは特定の条件が満たされる場合のみ制限的に実行できます。
        * where句が存在する時、術語(Predicate)がデータソースに完全にプッシュダウン(Pushdown)できる必要があります。
        * CLOB, NCLOB, BLOB, またはRAW(n)であるOracleタイプのカラムはプッシュダウンがサポートされません。
        * [詳細情報](https://trino.io/docs/476/connector/oracle.html#predicate-pushdown-support)
    * UPDATEは、特定の条件が満たされる場合のみ制限的に実行できます。
        * 定数値への割り当てと術語(Predicate)が存在する場合のみ実行することができます。
        * 算術式、関数呼び出しおよび定数以外の値へのUPDATE文はサポートされません。
        * テーブルの全てのカラムを同時に更新することはできません。
...
### EDBデータソースクエリの実行 { #execute-edb-data-source-query }

* EDBデータソースクエリは、Trino-PostgreSQLに基づいて実行されます。
...
* 制約事項
    * DELETEは特定の条件が満たされた場合のみ制限的に実行できます。
        ...
        * CHAR または VARCHAR のような文字列タイプに対する範囲条件(>, < または BETWEEN)はプッシュダウンがサポートされません。
        * テキストタイプに対する等価比較条件(IN, =, !=)はプッシュダウンがサポートされます。
        * [詳細情報](https://trino.io/docs/476/connector/postgresql.html#predicate-pushdown-support)
    * UPDATEは、特定の条件が満たされる場合のみ制限的に実行できます。
        * 定数値への割り当てと術語(Predicate)が存在する場合のみ実行することができます。
        * 算術式、関数呼び出しおよび定数以外の値へのUPDATE文はサポートされません。
        * テーブルの全てのカラムを同時に更新することはできません。
...
### MariaDBデータソースクエリ実行 { #execute-mariadb-data-source-query }
...
* 制約事項
    * DELETEは特定の条件が満たされた場合のみ制限的に実行できます。
        ...
        * テキストタイプのカラムはプッシュダウンがサポートされません。
        * [詳細情報](https://trino.io/docs/476/connector/mariadb.html#predicate-pushdown-support)
...
### Icebergデータソースクエリ実行 { #run-iceberg-data-source-queries }
...
* テーブルプロパティ
    * テーブルのメタデータを設定できます。 [追加情報](https://trino.io/docs/476/connector/iceberg.html#table-properties)

| プロパティ名 | 説明 |
| ----- | --- |
| format | テーブルデータファイルの形式を指定します。 PARQUET, ORC、またはAVRO. <br> デフォルト値はPARQUETです。 |
| partitioning | テーブルパーティションを指定します。テーブルがc1カラムとc2カラムに分割されている場合、partitioning=ARRAY['c1', 'c2']で指定できます。 |
...
* テーブルプロパティを使用して、テーブルデータが分割保存された構造の分割された(パーティションされた)テーブルを作成できます。
    * パーティションカラムがc1カラムとc2カラムに指定されている場合、該当パーティションのデータはテーブルデータパス下位の`/c1=<c1値>/c2=<c2値>`に保存されます。
* Icebergは書き込み(write)されたデータの値を通じてパーティションを自動的に管理してくれるので、パーティションを手動で追加/管理できません。
* テーブルカラムを利用(変換)してパーティションを指定できる機能をサポートします。
    * year, month, day, hour, bucket, truncate [追加情報](https://trino.io/docs/476/connector/iceberg.html#partitioned-tables)
...
* メタデータテーブルを照会してIcebergテーブルのメタ情報を確認できます。 [追加情報](https://trino.io/docs/476/connector/iceberg.html#metadata-tables)
    * $properties
        * テーブルプロパティ
    * $history
        * テーブルのメタデータが変更された履歴
...
* スキーマ進化(schema evolution)
    ...
    * カラムの追加、削除、再ソート、名前変更、タイプ変更をサポートします。
...
* add_files_with_partitionプロシージャ
  ...
  * 登録するパーティションカラムタイプがDATEの場合は`YYYY-MM-DD`、TIMESTAMPの場合は`YYYY-MM-DD HH:mm:ss`の形式で入力する必要があります。timezoneがあるTIMESTAMPの場合は`YYYY-MM-DD HH:mm:ss Asia/Seoul`のように最後にzoneIdを指定する必要があります。

    |引数 | サポートする値                  | 説明                                                                                                                                                                                    |
    | --- |---------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    | location | データファイルパス               | 追加したいデータファイルのパス                                                                                                                                                                       |
    | partition_columns | ARRAY['partition_column'] | icebergテーブルに定義されたパーティションカラムを羅列し、テーブルがc1、c2カラムに分割されている場合はARRAY['c1', 'c2']と入力 |
    ...
    |duplicate_file  | FAIL(デフォルト値), SKIP, ADD     | 登録しようとするデータファイルが既に登録されたicebergテーブルのデータファイルと重複している場合の動作<br>FAIL => icebergテーブルに既に登録されたデータファイルと比較して重複したデータファイルがある場合、クエリに失敗します<br>SKIP => 重複したファイルは無視します<br>ADD => データファイルを追加します。            |
```sql
-- パーティションカラムがyearであり、day変換が適用されたicebergテーブル
ALTER TABLE example.system.example_table 
...
## 外部連動 { #external-integration-2 }
<a id="trino-cli"></a>
### Trino CLI { #trino-cli }

* 設定メニューから発行された認証情報、接続情報、TrinoでサポートするCLIツールを利用してコマンドラインからクエリを実行できます。
  * DataQueryは現在Trino 476バージョンを基盤にサービスしています。
  * [Trino CLI](https://repo1.maven.org/maven2/io/trino/trino-cli/476/trino-cli-476-executable.jar)

```
# ファイルに実行権限が必要です。chmod +xで付与できます。
# 例) chmod +x trino-cli-476-executable.jar

./trino-cli-476-executable.jar --server <接続URL(必須)> \
  --user <ID(必須)> --password \
  --catalog <データソース名> \
  --schema <スキーマ名>
```
...
* 設定パラメータ
    * 接続URL(必須)
        * 設定画面で提供された接続URL(例：[https://x-x-x-x-x.kr1-cluster-dataquery.nhncloudservice.com](https://x-x-x-x-x.kr1-cluster-dataquery.nhncloudservice.com))
    * ID(必須)
        * 認証情報画面で提供されたID
    * パスワード(必須)
        * 認証情報画面で提供されたパスワード
...
* catalog、schema値はコマンドを実行する接続の値であり、入力しなくてもCLIを実行できます。また、以下のクエリでcatalogやschemaの一覧を確認できます。
    * show catalogs
    * show schemas
* 詳細は[Trinoガイドページ](https://trino.io/docs/476/client/cli.html)をご覧ください。

<a id="connect-to-jdbc"></a>
### JDBC接続 { #connect-to-jdbc }
...
* 設定パラメータ
    * host(必須)
        * 設定画面で提供された接続URLで`https://`以外の部分を入力します(例：x-x-x-x-x.kr1-cluster-dataquery.nhncloudservice.com)。
    * port(必須)
        * 443を入力します。
    * catalog
        * 接続したいデータソース名
...
### Cloud Scheduler連携 { #cloud-scheduler-connection }

* Cloud Schedulerサービスの**予約されたクエリ**テンプレートを使用して、クエリを任意のスケジュールで実行できます。

| パラメータ | 必須 | 説明 |
| ----- | --- | ---- |
| Appkey | O | DataQueryサービスのAppkey |
| リージョン | O | DataQueryサービスのリージョン |
| User Access Key | O | NHN Cloudが発行するUser Access Key |
...
!!! danger "注意"
    * クエリを実行するには、クラスターが起動している必要があります。
    * クエリを実行するには、DataQueryサービスの**認証情報**を発行した状態である必要があります。
    * User Access Key及びSecret Access Keyは、登録したAppkeyに権限を持つアカウントを使用して発行する必要があります。
